...
exports.getGameRatings = (req, res) => {
  try {
...
exports.getPopularRatings = (req, res) => {
  try {
    const ratings = db.prepare(`
...
exports.getPublicPlatformRatings = (req, res) => {
  try {
    const ratings = db.prepare(`
...
exports.getRatingsAdmin = (req, res) => {
  try {
    ...
    const ratings = db.prepare(`
      SELECT
        ...
        r.rating,
        r.review,
        COALESCE(r.is_hidden, 0) as is_hidden,
        r.created_at
      FROM game_ratings r
      JOIN users u ON r.user_id = u.id
    ...
    const platformRatings = db.prepare(`
      SELECT
        pr.id,
        u.name as user_name,
        u.phone as user_phone,
...
exports.hideRating = (req, res) => {
  try {
    const rating = db.prepare('SELECT id, is_hidden FROM game_ratings WHERE id = ?').get(req.params.id);
    if (!rating) return res.status(404).json({ error: 'Rating not found' });
    const nextHidden = rating.is_hidden ? 0 : 1;
    db.prepare('UPDATE game_ratings SET is_hidden = ? WHERE id = ?').run(nextHidden, req.params.id);
...
exports.deleteRating = (req, res) => {
  try {
    const result = db.prepare('DELETE FROM game_ratings WHERE id = ?').run(req.params.id);
